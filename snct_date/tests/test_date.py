...
class DateTest(TestCase):
    ...
    def provider_get_old_style_date():
        return (
            ('20.4.2014', '7.4.2014'),
            ('19.1.2014', '6.1.2014'),
            ('7.1.2015', '25.12.2014'),
    ...
    @data_provider(provider_get_old_style_date)
    def test_get_old_style_date(self, day_new, day_old):
        self.assertEquals(
            '.'.join(
                map(
                    str,
                    date.get_old_style_date(
                        *day_new.split('.')
                    )
                )
            ),
